...
| Model | Description | VRAM Requirement* |
| ----- | ----------- | ----------------- |
| `Qwen/Qwen-Image` | Qwen diffusion transformer, general-purpose | ~16 GiB |
| `Qwen/Qwen-Image-2512` | Qwen diffusion transformer, higher-resolution variant | ~16 GiB |
| `Tongyi-MAI/Z-Image-Turbo` | Tongyi MAI turbo diffusion model | ~16 GiB |
| `stepfun-ai/NextStep-1.1` | StepFun next-step diffusion model with extra CFG arguments | ~24 GiB |
| `meituan-longcat/LongCat-Image` | LongCat dual-stream diffusion transformer | ~24 GiB |
| `OvisAI/Ovis-Image` | Ovis-Image diffusion transformer | ~24 GiB |
| `OmniGen2/OmniGen2` | OmniGen2 multimodal generation model | ~24 GiB |
| `stabilityai/stable-diffusion-3.5-medium` | Stable Diffusion 3.5 Medium (2B parameters) | ~8 GiB |
| `black-forest-labs/FLUX.1-dev` | FLUX.1-dev (12B parameters) | ~24 GiB |
| `black-forest-labs/FLUX.2-klein-4B` | FLUX.2-klein 4B, efficient distilled variant | ~16 GiB |
| `black-forest-labs/FLUX.2-klein-9B` | FLUX.2-klein 9B, efficient distilled variant | ~24 GiB |
| `black-forest-labs/FLUX.2-dev` | FLUX.2-dev; weights exceed single-GPU 80 GiB capacity | >80 GiB (CPU offload required) |

*VRAM Requirement: approximate, based on basic single-card usage. FLUX.2-dev requires `--enable-cpu-offload` on a single 80 GiB GPU.
...
if __name__ == "__main__":
    ...
    for i, output in enumerate(outputs):
        output.request_output[0].images[0].save(f"{i}.jpg")
```

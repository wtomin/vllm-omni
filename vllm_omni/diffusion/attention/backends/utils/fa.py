...
def _upad_input(
    ...
    different batches. This function is used instead of `flash_attn.bert_padding.unpad_input` in order to avoid
    the recomputation of the same intermediary tensors for query, key, value tensors.
    ...
    # With static caches, the k/v states may be larger than the mask -> we need to slice them to
    # avoid generating garbage. It's a bit of an anti-pattern, but otherwise we silently compute
    # wrong attentions scores
